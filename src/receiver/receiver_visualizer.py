...
def on_message(client, userdata, message):
    ...
    try:
        ...
        ack_msg, delay = messages.process(message, config.SLAVE_ID)
        print(message)
        client.publish(config.TOPIC_MASTER_IN, json.dumps(ack_msg))
        if ack_msg and delay == None:
        ...
        if delay :
            ...
            # if(message.type == "BROADCAST_CONTROL" and message.control == "CHANGE_MUSIC"):
            if playback_thread and playback_thread.is_alive():
                stop_event.set()
                playback_thread.join()
            playback_thread = threading.Thread(target=delayed_action,args=(0, midi_pc.current_action))     
